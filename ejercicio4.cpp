...
public:
    ...
    void imprimirDestinos(int *destinos, int cantDestinos, int adyacente)
    {
        NodoLista<Arista> *aux = adyacentesA(adyacente);
        NodoLista<Arista> *ptr;
        for (int i = 0; i < cantDestinos; i++)
        {
            ptr = aux;
            while (ptr != NULL)
            {
                int valor = ptr->dato.destino;
                if (valor == destinos[i])
                {
                    cout << valor << " si" << endl;
                }
...
int main()
{
    ...
    bool *visitados = new bool[grafo->getV()]();
    for (int i = 0; i < grafo->getV(); i++)
    {
        visitados[i] = false;
    }
    grafo->bfs(1, grafo, visitados);

    for (int i = 0; i < cantDestinos; i++)
    {
        for (int j = 0; i < grafo->getV(); i++)
        {
             if (visitados[j] == destinosDeseados[i]){
                cout << visitados[j] << " si" << endl;
             }
        }
        cout << destinosDeseados[i] << " no" << endl;
    }
